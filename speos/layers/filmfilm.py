...
class FiLMFiLM(FiLMConv):
    """Test class to see if moving films into message method has identical results compared to computing it beforehand"""
    def __init__(self,
                 in_channels: int,
                 out_channels: int,
                 num_relations: int,
        ...
        super(FiLMFiLM, self).__init__(in_channels, out_channels=out_channels, num_relations=num_relations, nn=nn, act=act, aggr=aggr, **kwargs)

        self.films = ModuleList()
        for _ in range(num_relations):
            if nn is None:
                film = Linear(in_channels, 2 * out_channels)
            else:
                film = copy.deepcopy(nn)
            self.films.append(film)

        if nn is None:
            self.film_skip = Linear(in_channels, 2 * self.out_channels,
                                    bias=False)
        else:
    ...
    def message(self, x_j: Tensor, film, x_i) -> Tensor:
        beta_i, gamma_i = film(x_i).split(self.out_channels, dim=-1)
