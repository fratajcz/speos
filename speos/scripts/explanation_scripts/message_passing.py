...
class MessagePassing(torch.nn.Module):
    ...
    def add_types(self, types: Tensor):
        self._types = types
        self._types.requires_grad = False
    ...
    def explain_message(self, inputs: Tensor, size_i: int) -> Tensor:
        ...
        full_edge_mask = self._edge_mask

        if full_edge_mask is None:
        ...
        if not hasattr(self, "current_type"):
            self.current_type = 0
            #print("Created current type")
        if not hasattr(self, "unique_types"):
            unique_types = torch.unique(self._types)
            #print(unique_types)
            self.unique_types = unique_types

        actual_type = self.unique_types[self.current_type]
        edge_mask = full_edge_mask[self._types == actual_type]
        self.current_type += 1
        if actual_type == self.unique_types.max():
            self.current_type = 0

        #values, types = torch.tensor_split(edge_mask, 2, dim=1)
        #unique_types = torch.unique(types)
        #actual_type = unique_types[self.current_type]
        
        #edge_mask = values[types == actual_type]
        #self.current_type += 1
        #print("INcreased Current type")
        #print(types.max())
        #if actual_type == types.max():
        #    self.current_type = 0
        #    #print("Reset current type")
