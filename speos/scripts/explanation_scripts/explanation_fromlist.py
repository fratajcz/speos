...
parser.add_argument('--maxcs', "-x", type=int, default=11,
                    help='maximum cs of candidates to examine')
parser.add_argument('--readonly', "-r", action='store_true',
                    default=False,
                    help='if run should be readonly.')
parser.add_argument('--chunk', "-k", default=-1, type=int, 
                    help='The chunk of candidates that should be used.')
parser.add_argument('--nchunks', "-n", default=-1, type=int, 
                    help='The number of chunks the candidates should be split into')
...
ig_attr_node_ = None
if args.gene != "":
    genelist = pd.read_csv(args.gene, header=None, index_col=None, names=["hgnc"])["hgnc"].tolist()

    candidates = [dataset.preprocessor.hgnc2id[gene] for gene in genelist]
    genes = genelist
...
if args.gene == "" and args.index == -1:
    if args.mincs == 12:
        ...
        genes = [key for key, value in outer_results.items() if args.mincs <= value <= args.maxcs]

    if args.chunk > -1:
        genes = np.asarray(genes)
        genes = np.array_split(genes, args.nchunks)[args.chunk].tolist()
...
for i, (output_idx, gene) in enumerate(zip(candidates, genes)):
    ...
    torch.save(ig_attr_self_all, os.path.join("/lustre/groups/epigenereg01/projects/ppi-florin/explanations/",  '{}_ig_attr_self_total_{}.pt'.format(old_config_name, gene)))
    #torch.save(ig_attr_self_abs_all, os.path.join(config.pp.save_dir, '{}_ig_attr_self_{}.pt'.format(old_config_name, gene)))
    torch.save(ig_attr_edge_all, os.path.join("/lustre/groups/epigenereg01/projects/ppi-florin/explanations/", '{}_ig_attr_edge_total_{}.pt'.format(old_config_name, gene)))
    torch.save(ig_attr_node_all, os.path.join("/lustre/groups/epigenereg01/projects/ppi-florin/explanations/", '{}_ig_attr_node_total_{}.pt'.format(old_config_name, gene)))
